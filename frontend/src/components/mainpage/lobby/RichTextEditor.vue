<template>
    <QuillEditor v-model="content" theme="snow" toolbar="#custom-toolbar">
        <template #toolbar>
...
import content from "quill-image-uploader/src/blots/image";

// eslint-disable-next-line no-unused-vars
const Content = async () => {
    try {
        const response = await fetch('http://localhost:8080/delta/delta');
        const json = await response.json();
        content.value = JSON.parse(json).content;
    } catch (error) {
        console.error(error);
    }
};
